...
\]
\[
(x^T \Sigma_2^{-1} x - x^T \Sigma_1^{-1} x) - (\mu_2^T \Sigma_2^{-1} \mu_2 - \mu_1^T \Sigma_1^{-1} \mu_1) = 2 log(Q)
\]
Let
\[
S = 2log(Q) + (\mu_2^T \Sigma_2^{-1} \mu_2 - \mu_1^T \Sigma_1^{-1} \mu_1)
\]
\[
M = \Sigma_2^{-1} - \Sigma_1^{-1}
\]
Then we are finally left with solving
\[
x^T M x = T
\]
This is a quadratic form in d variables. It is all the points where the MH distance is equal to $T$. 
